with profile as (
    select * from {{ ref('mart__profile') }}
),

timeline as (
    select * from {{ ref('mart__timeline') }}
),

skills as (
    select * from {{ ref('mart__skills') }}
),

education as (
    select * from {{ ref('mart__education') }}
),

certifications as (
    select * from {{ source('raw', 'certification') }}
),

projects as (
    select * from {{ ref('mart__projects') }}
)
